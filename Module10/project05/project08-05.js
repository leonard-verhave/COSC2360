...
getLogButton.onchange = function() {
   ...
   fr.onload = function() {
       let game = JSON.parse(fr.result); // Ensure this is correct
       ...
       let mySet = new ChessSet(game); // Make sure to use 'new'
       setupBoard(mySet);
...
function showNextBoard(game) {
   ...
   moveSpans[currentMoveIndex].className = "highlight";
   
   // Read the notation for the next move 
   let moveStr = game.moves[currentMoveIndex];
...
function showPrevBoard(game) {
   ...
   moveSpans[currentMoveIndex].classList.remove("highlight");
